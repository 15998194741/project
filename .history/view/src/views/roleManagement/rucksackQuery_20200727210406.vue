    ...
    <div class="alertname">
      <el-table ref="multipleTable" :data="allselectchange">
        ...
        <el-table-column  label="回收数量" :width="widthtable">
          <template slot-scope="scope"><el-input v-model="scope.row.recycleAmount" :value="scope.row.articleAmount" placeholder="请输入内容"></el-input></template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    async filterFormClick() {
      ...
      this.tableDataBackup = this.tableData = tableDatas.map(item => {
        return { ...item, recycleAmount: '' };
      });
