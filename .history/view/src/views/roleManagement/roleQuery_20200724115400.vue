  ...
  methods: {
    ...
    exportFile() {
      require.ensure([], () => {
        ...
        const list = this.tableTrue; //把要导出的数据tableData存到list
    ...
    async findCharacter() {
      ...
      this.tableData.map(item =>{
        item.stime_etime = dayjs(item.stime).format('YYYY-MM-DD HH:mm:ss') + '----' + dayjs(item.stime).add(item.banned_time, 'hour');
        item.plaform = item.plaform ? item.plaform === '1' ? '安卓' : '苹果' : '';
        item.banned_time = item.banned_time + '小时';
