...
export default {
  ...
  methods: {
    ...
    async selectchange() {
      let { object_id, object_name, object_class } = this.filterForm;
      if (!(object_id && object_name && object_class)) {this.tableData = this.tableDataBackup;}
      this.tableData.filter(item => {
        let a = object_id && object_id === item.object_id;
        let b = object_id ? object_id === item.object_id : true;
        let c = object_class && object_class === item.object_class;
