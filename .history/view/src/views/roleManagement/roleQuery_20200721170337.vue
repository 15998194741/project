<template>
  ...
  <div class="role-container-search">
    <div class="server-container">ID：
      ...
      注册时间：
        <el-date-picker   v-model="filterForm['regtime']"  size='small' type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" >
        </el-date-picker>
        封禁时间：
        <el-date-picker   v-model="filterForm['stime']"  size='small' type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"  >
        </el-date-picker>
    </div>
   
    <div class="comprehensive-container">
      <div v-for='(i,index) in selectForm' :key='index'  class="select-item"  > {{i.label}}:
        <el-select v-model="filterForm[i.key]" :multiple="i['multiple']" placeholder="请选择" size='small' style="border-radius: 10px;" >
...
export default {
  ...
  async mounted() {
    ...
    this.selectForm[1].options = this.selectForm[1].options.concat(servername.data;
