  ...
  <el-dialog title="批量操作" :visible.sync="dialogFormchange"  :close-on-click-modal="false">
    ...
    <div class="alertname">
      <el-table ref="multipleTable" :data="allselectchange">
        <el-table-column prop='articleId' label="物品ID" :width="screenWidth">
        </el-table-column>
        <el-table-column prop="articleName" label="物品名称" :width="screenWidth">
        </el-table-column>
        <el-table-column prop="articleClass" label="物品类型" :width="screenWidth">
        </el-table-column>
        <el-table-column prop="articleAmount" label="持有数量" :width="screenWidth">
        </el-table-column>
        <el-table-column  label="回收数量" :width="screenWidth">
          <template slot-scope="scope"><el-input v-model="scope.row.recycleAmount" :value="scope.row.articleAmount" type='number'  :max='scope.row.articleAmount' :min='0' step='1'  placeholder="请输入内容"  @input="recycleAmountChange(scope.$index,scope.row)"></el-input></template>
        </el-table-column>
      </el-table>
    </div>
