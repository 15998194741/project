...
export default {
  ...
  methods: {
    ...
    async clickFilterFormChange() {
      ...
      let test = new Set(  
        tableDatas.map(item =>
          this.selectForm[0].options.find( ele => ele.label===item.object_id)?this.selectForm[0].options.push(({ 'label': item.object_id,'value': item.object_id })):item;
        ));
      console.log(test);
      
      this.selectForm[1].options.push(tableDatas.map(item =>   
