...
class rechargeService{
	...
	async query(data){
		...
		gameid =22222222;
		if(servername){
			let serverid = await dbSequelize.query(`select serverid from gm_server where  servername ='${servername}' `);
			serverid = serverid[0][0].serverid;		
		}
		let where = `where  appid ='${gameid}'`;
		console.log(+plaform === '1');
		where += !plaform ?  '' :+plaform === '2'?'platform = \'apple\' ':+plaform==='1'?' platform in (\'union\',\'alipay\',\'wechat\') ' :'';
