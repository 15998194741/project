...
class CharacterService{
	...
	async outputConsume(data){
		...
		let sql = `select roleid,object_id,object_method,object_name,object_type,object_number,timestamp from ${tablename} where roleid='${roleid}' ${object_type?`where object_type = '${object_type}'`:''}   offset ${pagesize*(page-1)} limit ${pagesize}`;

		let res =   await Ta.tasql(sql, token);
		let sqls = `select count(object_id) from ${tablename} where roleid='${roleid}' ${object_type?`where object_type = '${object_type}'`:''}  `;
