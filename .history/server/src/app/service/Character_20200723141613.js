...
class CharacterService{
	...
	async uploadFile(data, gameid){
		...
		let result =  reader.on('end', async()=>{
			let {tableKey, tableValue}= await CharacterService.readfilesData(filePath);
			let tablesql = `select * from gm_character  where gameid='${gameid}' and "${tableKey}" in (${tableValue.map(item => `'${item}'`)})`;
			var result = await CharacterService.byMany(tablesql);
			return  result;
		});
	...
	static async readfilesData(filePath){
