...
class CharacterService{
	...
	async queryByParms(data){
		...
		let gameid= 1;
		let token = await CharacterService.byOne(`select * from gm_game_token where gameid = '${gameid}' and type='character'`);
		console.log('token: ', token);
		let wheres = await CharacterService.byData({plaform, display, load, test});
		wheres += await CharacterService.byBetween(srttime);
		wheres += await CharacterService.byInArray({});
