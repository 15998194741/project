...
class CharacterService{
	...
	async queryByParms(data){
		...
		let pagesize = 1;
		let gameid= 1;
		let token = await CharacterService.byOne(`select token from gm_game_token where gameid = '${gameid}' and type='user'`);
		...
		// let sql = `select  *  from v_event_52  offset ${pagesize*(page-1)} limit ${pagesize}  `;
		let sql = 'select  *  from v_user_52    ';

		// token = 'JSy9U1IrxyUXMK08mdAKNWY5ZyadJZr6sR3lFQeSZDmmrLCjor2VmZ2RUPjKjsQ0';  //假token
