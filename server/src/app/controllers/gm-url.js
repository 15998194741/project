...
import gmUrlService from '../service/gm-url';
import statusCode from '../../utils/status-code';
@controller('/gm/url')
export class GmUrlController {
  @get('/findAll')
  async findAll(ctx) {
    ctx.log.resourceDesc = '查找全部数据';
    const result = await gmUrlService.findAll();
    ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @get('/findById/:id')
  async findById(ctx) {
    ctx.log.resourceDesc = '根据id查找数据';
    const result = await gmUrlService.findById(ctx.params.id);
    ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @post('/')
  async create(ctx) {
    ...
    const result = await gmUrlService.create(ctx.request.body);
  ...
  async update(ctx) {
    ...
    if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
    const result = await gmUrlService.updateById(ctx.request.body);
  ...
  async delete(ctx) {
    ...
    if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
    const result = await gmUrlService.logicDeleteByIdToUserId(ctx.request.body.id, ctx.user);
