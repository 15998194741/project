...
import gmUserService from '../service/gm-user';
...
export class GmUserController {
  @get('/findAll')
	async findAll(ctx) {
		...
		const result = await gmUserService.findAll();
		ctx.body = statusCode.SUCCESS_200('查找成功', result);
	}
  @get('/findById/:id')
  async findById(ctx) {
  	ctx.log.resourceDesc = '根据id查找数据';
  	const result = await gmUserService.findById(ctx.params.id);
  	ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @post('/')
  async create(ctx) {
  	...
  	const result = await gmUserService.create(ctx.request.body);
  ...
  async update(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmUserService.updateById(ctx.request.body);
  ...
  async delete(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmUserService.logicDeleteByIdToUserId(ctx.request.body.id, ctx.user);
