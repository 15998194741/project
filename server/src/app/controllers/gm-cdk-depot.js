...
import gmCdkDepotService from '../service/gm-cdk-depot';
import statusCode from '../../utils/status-code';
@controller('/gm/cdk/depot')
export class GmCdkDepotController {
  @get('/findAll')
  async findAll(ctx) {
    ctx.log.resourceDesc = '查找全部数据';
    const result = await gmCdkDepotService.findAll();
    ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @get('/findById/:id')
  async findById(ctx) {
    ctx.log.resourceDesc = '根据id查找数据';
    const result = await gmCdkDepotService.findById(ctx.params.id);
    ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @post('/')
  async create(ctx) {
    ...
    const result = await gmCdkDepotService.create(ctx.request.body);
  ...
  async update(ctx) {
    ...
    if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
    const result = await gmCdkDepotService.updateById(ctx.request.body);
  ...
  async delete(ctx) {
    ...
    if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
    const result = await gmCdkDepotService.logicDeleteByIdToUserId(ctx.request.body.id, ctx.user);
