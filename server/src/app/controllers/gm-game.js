...
import gmGameService from '../service/gm-game';
...
export class GmGameController {
  @get('/findAll')
	async findAll(ctx) {
		...
		const result = await gmGameService.findAll();
		ctx.body = statusCode.SUCCESS_200('查找成功', result);
	}
  @get('/findById/:id')
  async findById(ctx) {
  	ctx.log.resourceDesc = '根据id查找数据';
  	const result = await gmGameService.findById(ctx.params.id);
  	ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @post('/')
  async create(ctx) {
  	...
  	const result = await gmGameService.create(ctx.request.body);
  ...
  async update(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmGameService.updateById(ctx.request.body);
  ...
  async delete(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmGameService.logicDeleteByIdToUserId(ctx.request.body.id, ctx.user);
