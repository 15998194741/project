import { login, getInfo, loginOther, getPermission } from '@/api/user';
import { getToken, setToken, removeToken } from '@/utils/cookie-utils';
import { toeknsetuser, gameid } from '@/api/components.js';
...
const actions = {
  ...
  // 请求用户权限信息
  async getPermissionInfo({ commit }, params) {
    let { code, data } = await getPermission(params);
    // 请求成功时设置权限信息
    if (+code === 200) { commit('SET_PERMISSION_INFO', data); }
  }
};
